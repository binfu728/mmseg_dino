...
input_proj → ViT+Adapter independently, then features are mean-pooled
across time at each of the 4 output scales.
"""
import sys
...
class DINOv3TemporalBackbone(BaseModule):
    """DINOv3 ViT + Adapter, per-frame encoding with temporal mean pooling.

    Input:  (B, n_frames * in_bands, H, W)  e.g. (B, 120, 256, 256)
    Output: tuple of 4 feature maps at strides [4, 8, 16, 32],
            each (B, embed_dim, h_i, w_i).
    ...
    Args:
        ...
        in_bands: Number of bands per temporal frame (10 for full Sentinel-2).
            When in_bands != 3, a learnable 1x1 Conv projects to 3-channel RGB
            before the ViT, initialized with RGB bands copied through.
        n_frames: Number of temporal frames (12 for monthly median).
        init_cfg: Ignored.
    """
